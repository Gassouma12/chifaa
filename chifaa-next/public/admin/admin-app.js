// Chifaa Admin Panel JavaScript

// Authentication: server-side via api.php (signed HttpOnly cookie)
...
// Handle login - will be set up in main DOMContentLoaded
function setupLoginHandler() {
    ...
    console.log('[LOGIN] Attaching submit event listener to form...');
    ...
    newForm.addEventListener('submit', async (e) => {
        ...
        const newPasswordInput = document.getElementById('admin-password');
        const newStayConnected = document.getElementById('stay-connected');
        const newErrorMessage = document.getElementById('login-error-message');
        ...
        try {
            // Authenticate against the server, which sets the signed HttpOnly
            // cookie that api.php's write/upload actions require. A client-side
            // password check alone never sets that cookie, so saves would fail.
            const response = await fetch('api.php?action=login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ password: enteredPassword, remember: newStayConnected.checked })
            });
            const result = await response.json();

            if (response.ok && result.success) {
                console.log('[LOGIN] Authenticated! Hiding login screen...');
                const loginScreen = document.getElementById('admin-login-screen');
                loginScreen.classList.add('hidden');

                console.log('[LOGIN] Initializing admin panel...');
                await initializeAdminPanel();
                console.log('[LOGIN] Admin panel initialized!');
            } else {
                console.log('[LOGIN] Login failed:', result.error);
                newErrorMessage.textContent = result.error || "Incorrect password. Please try again.";
            ...
            newErrorMessage.textContent = "Could not reach the server. Please try again.";
            newErrorMessage.classList.add('show');
        }

        return false;
    });
    console.log('[LOGIN] Login handler setup complete!');
}
...
async function initializeAdminPanel() {
    console.log('[PANEL] Initializing admin panel...');
    try {
        console.log('[PANEL] Loading all data...');
        await loadAllData();
        console.log('[PANEL] Data loaded successfully');

        console.log('[PANEL] Initializing navigation...');
        initializeNavigation();

        console.log('[PANEL] Initializing buttons...');
        initializeButtons();

        console.log('[PANEL] Updating stats...');
        updateStats();

        console.log('[PANEL] Admin panel initialization complete!');
    } catch (error) {
        console.error('[PANEL] Error initializing admin panel:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log('[INIT] Is authenticated:', isAuthenticated);

    if (isAuthenticated) {
        // Already logged in - initialize admin panel
        console.log('[INIT] User authenticated, initializing admin panel...');
        await initializeAdminPanel();
    } else {
        // Not authenticated - show login screen and set up handler
        console.log('[INIT] User not authenticated, showing login screen...');
        const loginScreen = document.getElementById('admin-login-screen');
        console.log('[INIT] Login screen element:', !!loginScreen);
        if (loginScreen) {
            loginScreen.classList.remove('hidden');
            console.log('[INIT] Login screen classes:', loginScreen.className);
        }
        setupLoginHandler();
    }
...
async function loadAllData() {
    try {
        const files = ['home', 'about', 'aiCompanion', 'partners', 'authors', 'blog', 'founder', 'contact', 'team', 'podcast', 'menaHealthData'];
        ...
        for (const file of files) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`File ${file} not found, initializing empty`);
                    // Initialize with default structure
                    if (file === 'team' || file === 'podcast') {
                        currentData[file] = [];
                    } else if (file === 'menaHealthData') {
                        currentData[file] = [];
                    } else {
                ...
                try {
                    ...
                    if (file === 'team' || file === 'podcast' || file === 'menaHealthData') {
                        currentData[file] = [];
                    }
                }
            }
        }
        console.log('[DATA] All data loaded:', currentData);
    } catch (error) {
...
function normalizeAssetPath(url) {
    ...
    try {
        ...
        const assetsIndex = parsed.pathname.indexOf('/assets/');
        if (assetsIndex !== -1) {
            return parsed.pathname.slice(assetsIndex + 1);
        }
    } catch (error) {
        // Keep local relative values flowing through the simple cleanup below.
    }

    return String(url)
        .replace(/^(\.\.\/)+/, '')
        .replace(/^\/+/, '')
        .replace(/^admin\/assets\//, 'assets/');
}
...
function loadSection(section) {
    ...
    document.getElementById('section-title').textContent = sectionTitle[section] || section.charAt(0).toUpperCase() + section.slice(1);
    ...
    switch (section) {
        ...
        case 'founder':
        case 'founders':
            renderFounderSection(content);
            break;
        case 'team':
            renderItemsSection(content, 'team', getTeamFields());
            break;
        case 'podcast':
            renderItemsSection(content, 'podcast', getPodcastFields());
            break;
        case 'ai-companion':
            renderAiCompanionSection(content);
            break;
        case 'partners':
            renderItemsSection(content, 'partners', getPartnersFields());
            break;
        case 'contact':
            renderContactSection(content);
            break;
        default:
            content.innerHTML = '<div class="welcome-screen"><h2>Section not found</h2></div>';
    }
}
    ...
    container.innerHTML = `
        <div class="section-content">
            <div class="section-header">
                ...
                <p>Configure the homepage video and MENA map data.</p>
            </div>
            ...
            <div class="form-actions">
                <button class="btn-primary" onclick="saveHomeData()">💾 Save Video URL</button>
            </div>

            <hr style="margin: 40px 0; border: none; border-top: 2px solid var(--gray-200);">

            <div class="section-header">
                <h2>MENA Map Data</h2>
                <p>Edit health statistics for MENA region countries.</p>
                <button class="btn-primary" onclick="editMenaMapData()" style="margin-top: 16px;">📊 Edit Map Data</button>
            </div>
        </div>
    `;
}

// Render Founder Section
async function renderFounderSection(container) {
    // founder.json is an ARRAY of founders. The admin edits the first (primary)
    // founder in place and preserves any co-founders, so it must never collapse
    // the array back into a single object.
    let founderData = Array.isArray(currentData.founder) ? currentData.founder[0] : currentData.founder;

    // If founder data doesn't have the full content, try to fetch from the actual page
    if (!founderData || !founderData.fullBio) {
        try {
            ...
            if (response.ok) {
                ...
                // Extract data from the actual page
                const name = doc.querySelector('.founder-title')?.textContent.trim() || 'Maha Jouini';
                const subtitle = doc.querySelector('.founder-subtitle')?.textContent.trim() || 'AI GOVERNANCE EXPERT · FOUNDER OF CHIFAA · AUTHOR';
                const intro = doc.querySelector('.founder-intro')?.textContent.trim() || '';
                const bioElement = doc.querySelector('.founder-bio');
                const fullBio = bioElement ? bioElement.innerHTML : '';
                const image = normalizeAssetPath(doc.querySelector('.wavy-frame img')?.getAttribute('src') || 'assets/images/maha.png');

                // Extract tags
                const tags = Array.from(doc.querySelectorAll('.ftag')).map(tag => tag.textContent.trim());
                ...
                // Update currentData with fetched data, preserving any co-founders
                if (Array.isArray(currentData.founder)) {
                    currentData.founder[0] = founderData;
                } else {
                    currentData.founder = founderData;
                }
            }
        } catch (error) {
            console.error('Error fetching founder page:', error);
        }
    }

    // Fallback to basic structure if still no data
    if (!founderData) {
        founderData = {
            name: 'Maha Jouini',
            subtitle: 'Founder & AI Advocate',
            intro: '',
            fullBio: '',
    ...
    const founderImagePreview = adminPreviewSrc(founderData.image);

    container.innerHTML = `
        <div class="section-content">
            <div class="section-header">
                <h2>The Founder</h2>
                <p>Configure information about the founder. This content appears on the founder page.</p>
            </div>
            
            <div class="form-group">
                <label class="form-label">Name</label>
                <input type="text" class="form-input" id="founder-name" 
                    value="${escapeHtml(founderData.name || '')}" 
                    placeholder="Founder's Name" 
                    onchange="updateFounderData('name', this.value)">
            </div>

            <div class="form-group">
                <label class="form-label">Subtitle</label>
                <input type="text" class="form-input" id="founder-subtitle" 
                    value="${escapeHtml(founderData.subtitle || '')}" 
                    placeholder="AI GOVERNANCE EXPERT · FOUNDER OF CHIFAA · AUTHOR" 
                    onchange="updateFounderData('subtitle', this.value)">
            </div>

            <div class="form-group">
                <label class="form-label">Quote/Introduction</label>
                <textarea class="form-textarea" id="founder-intro" rows="4"
                    placeholder="Opening quote or introduction..." 
                    onchange="updateFounderData('intro', this.value)">${escapeHtml(founderData.intro || '')}</textarea>
                <small class="form-help">This appears as the introductory quote on the founder page</small>
            </div>

            <div class="form-group">
                <label class="form-label">Full Biography (HTML)</label>
                <textarea class="form-textarea" id="founder-fullBio" rows="15"
                    placeholder="<p>Full biography with HTML formatting...</p>" 
                    onchange="updateFounderData('fullBio', this.value)">${escapeHtml(founderData.fullBio || '')}</textarea>
                <small class="form-help">Full biography content with HTML tags. This is the main content of the founder page.</small>
            </div>

            <div class="form-group">
                <label class="form-label">Profile Image Path</label>
                <input type="text" class="form-input" id="founder-image" 
                    value="${escapeHtml(normalizeAssetPath(founderData.image || ''))}" 
                    placeholder="assets/images/maha.png" 
                    onchange="updateFounderData('image', this.value)">
                ${founderImagePreview ? `<img src="${escapeHtml(founderImagePreview)}" alt="Founder" style="max-width: 200px; margin-top: 10px; border-radius: 8px;">` : ''}
                <div style="margin-top: 10px;">
                    <button type="button" class="btn-secondary" onclick="uploadFounderImage()">Upload Founder Image</button>
                </div>
                <small class="form-help">Path relative to the root directory</small>
            </div>

            <div class="form-group">
                <label class="form-label">Tags (comma-separated)</label>
                <input type="text" class="form-input" id="founder-tags" 
                    value="${(founderData.tags || []).join(', ')}" 
                    placeholder="AI Governance, AUDA-NEPAD, AFRIA VP, Author" 
                    onchange="updateFounderData('tags', this.value.split(',').map(t => t.trim()))">
                <small class="form-help">Tags that appear at the bottom of the founder page</small>
            </div>

            <div class="form-actions">
                <button class="btn-primary" onclick="saveFounderData()">💾 Save Changes</button>
                <button class="btn-secondary" onclick="location.reload()">🔄 Reload from Page</button>
            </div>
        </div>
    `;
}

// Render AI Companion Section
function renderAiCompanionSection(container) {
    const data = currentData.aiCompanion || { title: '', description: '', features: [] };

    container.innerHTML = `
        <div class="section-content">
            <div class="section-header">
                <h2>AI Companion Settings</h2>
                <p>Configure the AI Companion information.</p>
            </div>
            
            <div class="form-group">
                <label class="form-label">Title</label>
                <input type="text" class="form-input" id="ai-companion-title" 
                    value="${data.title || ''}" 
                    placeholder="AI Companion Title" 
                    onchange="updateAiCompanionData('title', this.value)">
            </div>

            <div class="form-group">
                <label class="form-label">Description</label>
                <textarea class="form-textarea" id="ai-companion-description" rows="6"
                    placeholder="Describe the AI Companion..." 
                    onchange="updateAiCompanionData('description', this.value)">${data.description || ''}</textarea>
            </div>

            <div class="form-actions">
                <button class="btn-primary" onclick="saveAiCompanionData()">💾 Save Changes</button>
            </div>
        </div>
    `;
}
...
function renderContactSection(container) {
    const data = currentData.contact || { email: '', phone: '', address: '' };

    container.innerHTML = `
        <div class="section-content">
            <div class="section-header">
                <h2>Contact Information</h2>
                <p>Configure contact details.</p>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-input" id="contact-email" 
                    ...
                    onchange="updateContactData('email', this.value)">
            </div>

            <div class="form-group">
                <label class="form-label">Phone</label>
                <input type="text" class="form-input" id="contact-phone" 
                    value="${data.phone || ''}" 
                    placeholder="+216 XX XXX XXX" 
                    onchange="updateContactData('phone', this.value)">
            </div>

            <div class="form-group">
                <label class="form-label">Address</label>
                <textarea class="form-textarea" id="contact-address" rows="3"
                    placeholder="Full address..." 
                    onchange="updateContactData('address', this.value)">${data.address || ''}</textarea>
            </div>

            <div class="form-actions">
                <button class="btn-primary" onclick="saveContactData()">💾 Save Changes</button>
            </div>
        </div>
    `;
}
            ...
            if (field.key === 'content' && quillEditor) {
                ...
                if (input) {
                    ...
                    if (field.key.includes('.')) {
                        ...
                        currentData[section][index][field.key] = normalizeAssetPath(input.value);
                    } else {
...
async function uploadFounderImage() {
    ...
    input.onchange = async () => {
        ...
        try {
            showToast('Uploading founder image...', 'warning');
            const formData = new FormData();
            formData.append('section', 'founder');
            ...
            updateFounderData('image', normalizeAssetPath(result.url));
            renderFounderSection(document.getElementById('admin-content'));
            showToast('Founder image uploaded. Click "Save Changes" to persist.', 'success');
        } catch (error) {
...
async function uploadCroppedImage() {
    ...
    try {
        ...
        if (savedImageType === 'cover') {
            ...
            if (result.success && result.url) {
                const uploadedUrl = normalizeAssetPath(result.url);
                ...
                if (input) {
                    ...
                    if (container) {
                        ...
                        preview.innerHTML = `<img src="${adminPreviewSrc(uploadedUrl)}" alt="Preview"><button type="button" class="remove-image" onclick="removeUploadedImage('${savedFieldKey}')">×</button>`;
            ...
            if (result.success && result.url) {
                ...
                // Insert image
                // Insert image
                quillEditor.insertEmbed(savedRange.index, 'image', uploadedUrl);
...
function renderFormField(field, item) {
    ...
    if (field.key === 'content' && field.type === 'textarea') {
        ...
        return `
            <div class="form-group">
                ...
                <div class="image-upload-container">
                    ...
                    ${val ? `<div class="image-preview"><img src="${escapeHtml(adminPreviewSrc(val))}" alt="Preview"><button type="button" class="remove-image" onclick="removeUploadedImage('${field.key}')">×</button></div>` : ''}
        ...
        const options = field.options || [];
        // A single fixed option (e.g. the author) is always selected — no blank placeholder.
        const single = options.length === 1;
        const optionsHtml = options.map(opt =>
            `<option value="${escapeHtml(opt)}" ${(val === opt || (single && !val)) ? 'selected' : ''}>${opt}</option>`
        ).join('');
        return `
            <div class="form-group">
                <label class="form-label">${field.label}</label>
                <select class="form-select" name="${field.key}">
                    ${single ? '' : `<option value="">Select ${field.label}</option>`}
                    ${optionsHtml}
                </select>
                ${helpText}
            </div>
        ...
        const imagesHtml = imageArray.map((url, index) => `
            <div class="multi-image-item" data-index="${index}">
                <img src="${adminPreviewSrc(url)}" alt="Image ${index + 1}">
...
// Update founder data
function updateFounderData(key, value) {
    const v = key === 'image' ? normalizeAssetPath(value) : value;
    // Edit the primary founder (array[0]) in place, preserving co-founders.
    if (Array.isArray(currentData.founder)) {
        if (!currentData.founder[0]) currentData.founder[0] = {};
        currentData.founder[0][key] = v;
    } else {
        if (!currentData.founder) currentData.founder = {};
        currentData.founder[key] = v;
    }
    hasChanges = true;
}
...
async function saveFounderData() {
    try {
        const response = await fetch('api.php?action=write&file=founder', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(currentData.founder)
        });
        ...
        if (response.ok) {
            ...
            throw new Error('Failed to save founder data');
        }
    } catch (error) {
        showToast('Error saving founder data: ' + error.message, 'error');
    }
}

// Update AI Companion data
function updateAiCompanionData(key, value) {
    if (!currentData.aiCompanion) {
        currentData.aiCompanion = {};
    }
    currentData.aiCompanion[key] = value;
    hasChanges = true;
}

// Save AI Companion data
async function saveAiCompanionData() {
    try {
        const response = await fetch('api.php?action=write&file=aiCompanion', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(currentData.aiCompanion)
        });

        if (response.ok) {
            showToast('AI Companion settings saved successfully!', 'success');
            hasChanges = false;
        } else {
            throw new Error('Failed to save AI Companion data');
        }
    } catch (error) {
        showToast('Error saving AI Companion data: ' + error.message, 'error');
    }
}
...
async function saveContactData() {
    try {
        const response = await fetch('api.php?action=write&file=contact', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(currentData.contact)
        });
        ...
        showToast('Error saving contact data: ' + error.message, 'error');
    }
}

// Edit MENA Map Data
function editMenaMapData() {
    if (!currentData.menaHealthData || currentData.menaHealthData.length === 0) {
        showToast('No MENA map data loaded', 'error');
        return;
    }

    const modal = createModal('Edit MENA Map Data', () => {
        const countriesHtml = currentData.menaHealthData.map((country, index) => `
            <div class="form-section" style="border: 2px solid var(--gray-200); padding: 20px; margin-bottom: 20px; border-radius: 8px;">
                <h4 class="form-section-title">${country.flag} ${country.country}</h4>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Country ID</label>
                        <input type="text" class="form-input" value="${country.id}" readonly style="background: var(--gray-100);">
                    </div>
                    <div class="form-group">
                        <label class="form-label">ISO2 Code</label>
                        <input type="text" class="form-input" value="${country.iso2}" readonly style="background: var(--gray-100);">
                    </div>
                </div>
                <h5 style="margin-top: 20px; color: var(--color-accent);">Women's Health</h5>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Female Population Share</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="womensHealth" data-field="femalePopShare" value="${country.womensHealth.femalePopShare}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Life Expectancy</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="womensHealth" data-field="lifeExpectancy" value="${country.womensHealth.lifeExpectancy}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Maternal Mortality</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="womensHealth" data-field="maternalMortality" value="${country.womensHealth.maternalMortality}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Literacy Rate</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="womensHealth" data-field="literacyRate" value="${country.womensHealth.literacyRate}">
                    </div>
                </div>
                <h5 style="margin-top: 20px; color: var(--color-accent);">Chronic Disease</h5>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Diabetes Prevalence</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="chronicDisease" data-field="diabetesPrevalence" value="${country.chronicDisease.diabetesPrevalence}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Cardiovascular Rate</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="chronicDisease" data-field="cardiovascularRate" value="${country.chronicDisease.cardiovascularRate}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">Obesity Rate</label>
                    <input type="text" class="form-input mena-field" data-index="${index}" data-category="chronicDisease" data-field="obesityRate" value="${country.chronicDisease.obesityRate}">
                </div>
                <h5 style="margin-top: 20px; color: var(--color-accent);">Cervical Cancer</h5>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Incidence Rate</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="cervicalCancer" data-field="incidenceRate" value="${country.cervicalCancer.incidenceRate}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Mortality Rate</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="cervicalCancer" data-field="mortalityRate" value="${country.cervicalCancer.mortalityRate}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">HPV Vac Coverage</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="cervicalCancer" data-field="hpvVacCoverage" value="${country.cervicalCancer.hpvVacCoverage}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Screening Coverage</label>
                        <input type="text" class="form-input mena-field" data-index="${index}" data-category="cervicalCancer" data-field="screeningCoverage" value="${country.cervicalCancer.screeningCoverage}">
                    </div>
                </div>
            </div>
        `).join('');
        ...
        // Save callback
        const fields = document.querySelectorAll('.mena-field');
        fields.forEach(field => {
            const index = parseInt(field.dataset.index);
            const category = field.dataset.category;
            const fieldName = field.dataset.field;
            currentData.menaHealthData[index][category][fieldName] = field.value;
        });
        hasChanges = true;
        saveMenaMapData();
    });
...
// Save MENA Map Data
async function saveMenaMapData() {
    try {
        const response = await fetch('api.php?action=write&file=menaHealthData', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(currentData.menaHealthData)
        });

        if (response.ok) {
            showToast('MENA map data saved successfully!', 'success');
            hasChanges = false;
        } else {
            throw new Error('Failed to save MENA map data');
        }
    } catch (error) {
        showToast('Error saving MENA map data: ' + error.message, 'error');
    }
}
...
function exportAllData() {
    ...
    link.href = url;
    link.download = `Chifaa-backup-${Date.now()}.json`;
    link.click();
...
function updateStats() {
    ...
    if (statExperts) statExperts.textContent = currentData.team?.length || 0;
    if (statServices) statServices.textContent = currentData.podcast?.length || 0;
    if (statStartups) statStartups.textContent = currentData.founder ? 1 : 0;
...
function getFieldsForSection(section) {
    ...
    switch (section) {
        ...
        case 'podcast': return getPodcastFields();
        default:
...
function getBlogFields() {
    return [
        ...
        // All articles are authored by Maha Jouini by default (single fixed author).
        { key: 'author', label: 'Author', type: 'select', section: 'Author', options: ['Maha Jouini'], help: 'Articles are published under Maha Jouini' },
        ...
        { key: 'category', label: 'Categories', type: 'multiselect', section: 'Classification', options: ['Treatment Advice', 'Advocacy', 'Maha Jouini Stories', 'Community Stories', 'Research & Insights', 'Prevention & Screening', 'Survivor Support'], help: 'Select one or more categories' },
...
function getTeamFields() {
    return [
        { key: 'id', label: 'ID', type: 'number', section: 'Basic Info' },
        { key: 'name', label: 'Name', type: 'text', isTitle: true, section: 'Basic Info', placeholder: 'Team member name' },
        { key: 'role', label: 'Role', type: 'text', section: 'Basic Info', placeholder: 'Position/Role' },
        { key: 'eyebrow', label: 'Modal Role Label', type: 'text', section: 'Basic Info', placeholder: 'Optional alternate role label' },
        { key: 'bio', label: 'Biography', type: 'textarea', section: 'Info', rows: 4, placeholder: 'Brief bio...' },
        { key: 'image', label: 'Profile Picture', type: 'image-upload', section: 'Images', help: 'Upload team member photo' },
        { key: 'email', label: 'Email', type: 'email', section: 'Contact', placeholder: 'email@example.com' },
        { key: 'linkedin', label: 'LinkedIn URL', type: 'url', section: 'Social', placeholder: 'https://linkedin.com/in/...' },
        { key: 'twitter', label: 'Twitter URL', type: 'url', section: 'Social', placeholder: 'https://twitter.com/...' },
        { key: 'instagram', label: 'Instagram URL', type: 'url', section: 'Social', placeholder: 'https://instagram.com/...' },
        { key: 'behance', label: 'Behance URL', type: 'url', section: 'Social', placeholder: 'https://behance.net/...' },
        { key: 'founderLink', label: 'Show Founder Bio Link', type: 'boolean', section: 'Links' }
    ];
}

// Podcast fields
function getPodcastFields() {
    return [
        { key: 'id', label: 'ID', type: 'number', section: 'Basic Info' },
        { key: 'title', label: 'Episode Title', type: 'text', isTitle: true, section: 'Basic Info', placeholder: 'Episode 1: The Future of AI' },
        { key: 'description', label: 'Description', type: 'textarea', section: 'Content', rows: 4, placeholder: 'Episode description...' },
        { key: 'youtubeUrl', label: 'YouTube Video URL', type: 'url', section: 'Media', placeholder: 'https://www.youtube.com/watch?v=...', help: 'Full YouTube video URL' },
        { key: 'tag', label: 'Category Tag', type: 'text', section: 'Metadata', placeholder: 'Mental Health, Youth Voices, etc.' },
        { key: 'publishedDate', label: 'Published Date', type: 'date', section: 'Metadata', help: 'Format: YYYY-MM-DD' },
        { key: 'featured', label: 'Featured Episode', type: 'boolean', section: 'Metadata' }
    ];
}
...
async function handleMultiImageUpload(fieldKey, files) {
    ...
    for (let file of files) {
        try {
            ...
            const result = await response.json();
            if (result.success) {
                currentImages.push(normalizeAssetPath(result.url));
            } else {
